...
{
	int activeThreads = 0;
	//put the threads on hold for clients
	for (int i = 0; i < Max_Nr_Threads; i++)
		m_threads.emplace_back(std::thread(&Server::ThreadFunction, this));

	while (m_keepRunning)
	{
		std::unique_lock<std::mutex> lock(m_waitFreeThreads); //needed this to use the availableThreads.wait(), not sure if it is a problem yet

		ClientPtr client = ClientPtr(listener.Accept());

		if (activeThreads >= Max_Nr_Threads) //if we already have the maximum number of threads,
		{
			m_availableThreads.wait(lock);//wait for a thread to become available 
			--activeThreads;
		}

		//locked so only this thread can use the queue m_clients
		m_workMutex.lock();
		m_clients.push(std::move(client));
		m_workMutex.unlock();

		m_canWork.notify_one();//notify a thread there is stilll some work to do
		++activeThreads;
	}
}
...
void Server::acceptRequest(ClientPtr client, std::mutex& m)
{
	...
	while (1)
	{
		...
		//std::cout << messageToClient.dump(4) << "\n";
		...
		std::cout << "Package sent to socket: " << client->getSocketName() <<
			" in " << std::chrono::duration_cast<std::chrono::milliseconds> (end - start).count() << " miliseconds \n";
	}
}
...
void Server::ThreadFunction()
{
	while (1)
	{
		std::unique_lock<std::mutex> lock(m_workMutex); //lock the work mutex so only the first thread to call this can acces the queue
		ClientPtr client = nullptr;

		m_availableThreads.notify_all();//notify when it is available

		//condition.wait() //wait until another thread tells this one that a job is ready
		if (m_clients.empty())
			m_canWork.wait(lock);
